...
async def convert_from_roman(input):
    ...
    if output is None:
        raise HTTPException(status_code=400, detail=f"Conversion failed. Output:{output}")
    ...
    try:
        storage.create_table_if_not_exists(PG_SCHEMA, PG_POST_TABLE, column_types, add_pk=True)
        storage.insert(PG_SCHEMA, PG_POST_TABLE, payload)
        logger.info(f"Data inserted successfully into {PG_SCHEMA}.{PG_POST_TABLE}", payload=payload)
